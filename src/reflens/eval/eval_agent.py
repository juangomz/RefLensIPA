...
from reflens.eval.eval_prompts import EVAL_SYSTEM, EVAL_SCHEMA_HINT

Verdict = Literal["pass", "revise", "reject"]
Chunk = Dict[str, Any]  # {"id": "...", "text": "...", "score": 0.8, ...}
ALLOWED_SCORE_VALUES = (0.0, 0.25, 0.5, 0.75, 1.0)
SCORE_KEYS = (
...
class EvalAgent:
    ...
    def review(
        ...
        # Ensure support_type exists in citations for downstream analysis/debug.
        citations = eval.get("citations")
        if isinstance(citations, list):
            for c in citations:
                if not isinstance(c, dict):
                    continue
                st = str(c.get("support_type", "")).strip().lower()
                if st not in {"grounded", "parametric", "mixed"}:
                    supports = c.get("supports")
                    c["support_type"] = "grounded" if supports else "parametric"
        else:
            eval["citations"] = []

        # Calibrated verdict guardrail (grounded-preferred mode):
        # pass requires enough grounded faithfulness and no high-severity issues.
        ...
        if isinstance(issues, list):
            high_issues = sum(
                1
                for it in issues
                if isinstance(it, dict) and str(it.get("severity", "")).lower() == "high"
            )
        grounded_faithfulness = float(eval_scores.get("grounded_faithfulness", eval_scores.get("faithfulness", 0.0)))

        verdict: Verdict = eval.get("verdict", "revise")
        if verdict == "pass" and (grounded_faithfulness < 0.75 or high_issues > 0):
            verdict = "revise"
